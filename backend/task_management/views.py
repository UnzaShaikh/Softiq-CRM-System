...
class TaskViewSet(viewsets.ModelViewSet):
    queryset = Task.objects.select_related(
        'assignee', 'created_by', 'updated_by', 'related_content_type'
    ).prefetch_related('tags', 'checklist_items', 'attachments').all()

    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_class = TaskFilter
    search_fields = ['title', 'description']
    ordering_fields = ['created_at', 'due_date', 'priority', 'status', 'assignee__username', 'title']
    ordering = ['-created_at']
    ...
    def kanban(self, request):
        ...
        for status_value in statuses:
            qs = base_qs.filter(status=status_value)
    ...
    def update_status(self, request, pk=None):
        ...
        if new_status not in valid_statuses:
            return Response(
                {'status': f"Must be one of {list(valid_statuses)}."},
