...
export default function NotificationsPage() {
  const [prefs, setPrefs] = useState({
    ...
    newLead: true, dealUpdates: true, taskReminders: true,
    weeklyReport: false, systemAlerts: true,
  });
  ...
  const GROUPS = [
    ...
    {
      title: "Activity Notifications", items: [
            ...
            <div style={{ padding: "24px" }}>
              ...
              {GROUPS.map((group, gi) => (
                <div key={group.title} style={{ marginBottom: gi < GROUPS.length - 1 ? "24px" : 0 }}>
                  <h3 style={{ margin: "0 0 14px", fontSize: "0.875rem", fontWeight: 700, color: "#374151", paddingBottom: "8px", borderBottom: "1px solid #f1f5f9" }}>{group.title}</h3>
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                    {group.items.map(item => (
                      <div key={item.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0" }}>
                        ...
                        <Toggle value={prefs[item.key as keyof typeof prefs] as boolean} onChange={() => setPrefs(p => ({ ...p, [item.key]: !p[item.key as keyof typeof p] }))} />
                      </div>
                    ))}
                  </div>
                </div>
