...
import notesData, { Note, CATEGORY_COLORS, PRIORITY_COLORS } from "@/data/notes";
import { ArrowLeft, Edit, Star, MoreHorizontal, FileText, Tag, Calendar, Clock, Building2, ThumbsUp, ThumbsDown, Trash2 } from "lucide-react";
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: "12px", padding: "20px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {[
                  { label: "Category", value: <span style={{ padding: "2px 8px", borderRadius: "5px", fontSize: "0.8rem", fontWeight: 600, background: catStyle.bg, color: catStyle.color }}>{note!.category}</span>, icon: <Tag size={14} color="#94a3b8" /> },
                  { label: "Tags", value: <div style={{ display: "flex", gap: "4px", flexWrap: "wrap" }}>{note!.tags.map(t => <span key={t.id} style={{ padding: "2px 8px", borderRadius: "9999px", fontSize: "0.72rem", background: "#eef2ff", color: "#4f46e5", fontWeight: 500 }}>{t.label}</span>)}</div>, icon: <Tag size={14} color="#94a3b8" /> },
                  ...(note!.relatedTo ? [{ label: "Related To", value: <span style={{ color: "#4f46e5", fontSize: "0.875rem", fontWeight: 500, display: "flex", alignItems: "center", gap: "4px" }}><Building2 size={13} />{note!.relatedTo}</span>, icon: <Building2 size={14} color="#94a3b8" /> }] : []),
                  { label: "Created", value: <span style={{ fontSize: "0.8125rem", color: "#374151" }}>{formatDate(note!.createdAt)}</span>, icon: <Calendar size={14} color="#94a3b8" /> },
                  { label: "Last Updated", value: <span style={{ fontSize: "0.8125rem", color: "#374151" }}>{formatDate(note!.updatedAt)}</span>, icon: <Clock size={14} color="#94a3b8" /> },
                ].map(row => (
                  <div key={row.label}>
