...
// Backend priority ("low"/"medium"/"high")
// -> existing frontend NotePriority labels
const PRIORITY_FROM_API: Record<
  ApiNote["priority"],
  NotePriority
> = {
  low: "Low Priority",
...
// Matches a backend category name against
// the existing fixed NoteCategory enum.
function matchCategory(
  name: string | undefined
): NoteCategory {
  if (
    name &&
    (ALL_CATEGORIES as string[]).includes(name)
  ) {
...
/** Converts backend ApiNote into existing frontend Note shape. */
export function mapApiNoteToUi(
  apiNote: ApiNote,
  categories: ApiNoteCategory[] = []
): Note {
  const categoryObj = categories.find(
    (c) => c.id === apiNote.category
  );
  ...
  return {
    ...
    category:
      matchCategory(categoryObj?.name),
...
export function getNotesSummary(): Promise<NotesSummary> {
  return apiRequest<NotesSummary>(
    `/api/notes/summary/`
  );
}

export function getNoteOptions(): Promise<NoteOptions> {
  return apiRequest<NoteOptions>(
    `/api/notes/options/`
  );
}
...
> {
  const res =
    await apiRequest<
      | ApiNoteCategory[]
      | PaginatedResponse<ApiNoteCategory>
    >(
      `/api/note-categories/`
    );
...
export function createCategory(
  name: string
): Promise<ApiNoteCategory> {
  return apiRequest<ApiNoteCategory>(
    `/api/note-categories/`,
    {
      method: "POST",
      body: { name },
